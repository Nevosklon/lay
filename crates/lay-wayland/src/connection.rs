...
impl Connection {
    pub fn from_env() -> Result<Self, ConnError> {
        log::info!(target: "socket", "Attempt to connecting to wayland socket");
        ...
        if let Some(xdg_path) = env::var_os("XDG_RUNTIME_DIR") {
            ...
            log::info!(target: "socket", "Attempting to connect with XDG_RUNTIME_DIR + wayland-0: {path:?}");
            //
        ...
        if let Some(socket) = env::var_os("WAYLAND_SOCKET") {
            log::warn!(target: "socket", "Attempting to connect with WAYLAND_SOCKET");
